...
namespace StellarMass.InputManagement.Editor
{
    public static class MapInstanceGenerator
    {
        ...
        private static void WriteToFile(InputActionMap map)
        {
            ...
            {
                Debug.LogWarning($"File {filePathToWrite} already exists. Skipping write operation.");
                return;
            }
        ...
        private static void AddContent(string markerName, InputActionMap map, List<string> lines)
        {
            ...
            void className()
            {
                ...
                lines.Add($"    public class {mapName} : {mapInstanceName},");
            }

            void interfaceName()
            {
                lines.Add($"    {inputActionsGeneratedName}.I{mapName}Actions");
            }
            ...
            void actionsPropertyName()
            {
                lines.Add($"            {mapName}Actions");
            }
            ...
            void interfaceMethods()
            {
                foreach (InputAction inputAction in map.actions)
                {
                    ...
                    lines.Add($"        void {inputActionsGeneratedName}.I{mapName}Actions.On{trimmedName}(InputAction.CallbackContext context) => On{trimmedName}?.Invoke({argumentsString});");
                }
            }
        }
    }
